...
function SubjectAvgCard({
  ...
  const pct = maxMarks
    ? (item.avg / maxMarks) * 100
    : (item.avg / item.max_marks) * 100;
  const barColor =
  ...
  if (onPress)
    return (
      <Pressable style={styles.subjectAvgCard} onPress={onPress}>
        {content}
      </Pressable>
    );
...
function ClassExamView({
  data,
  examId,
}: {
  data: ExamOverviewStaffClass;
  examId: string;
}) {
  return (
    <ScrollView
...
function SectionExamView({
  data,
  examId,
}: {
  data: ExamOverviewStaffSection;
  examId: string;
}) {
  return (
    <ScrollView
    ...
    >
      <View style={styles.sectionBadgeContainer}>
        <Text style={styles.sectionBadgeText}>
          {data.exam.section ? `Section ${data.exam.section.name}` : data.exam.exam_name}
        </Text>
      </View>
      ...
      <View style={styles.card}>
        {(data.subject_avgs ?? []).map((item) => (
          <SubjectAvgCard
            ...
            onPress={() =>
              router.push(
                `/(tabs)/principal/question-heatmap?exam_id=${examId}&section_id=${data.exam.section?.id}&subject_id=${item.subject_id}` as any,
              )
            }
          />
        ))}
    ...
    >
      <View style={styles.studentInfoCard}>
        ...
        <Text style={styles.studentInfoBody}>
          This exam overview was returned in student mode. The principal flow
          can display it safely, but staff drill-down navigation is not
        ...
        {data.student_results.subjects.map((subject) => (
          <View key={subject.subject_id} style={styles.studentSubjectCard}>
            ...
            <View style={styles.studentSubjectMetaRow}>
              ...
              <Text
                style={[
                  styles.studentSubjectMeta,
                  { color: RISK_CFG[subject.risk_label].color },
                ]}
              >
                {subject.risk_label}
              </Text>
