...
import { analyticsApi, type ExamSummary, type AnalyticsStatus } from '../../../services/analytics';
import { useTeacherStore } from '../../../store/teacher-store';

const ACCENT = '#185FA5';

const STATUS_CONFIG: Record<AnalyticsStatus, { color: string; bg: string; label: string }> = {
      ...
      >
        ...
        {!loading && !error && doneExams.length > 0 && (
          <>
            ...
            {doneExams.map(exam => <ExamCard key={exam.id} exam={exam} />)}
          </>
        )}
...
function ExamCard({ exam }: { exam: ExamSummary }) {
  ...
  return (
    <Pressable
      style={({ pressed }) => [styles.card, pressed && isDone && styles.cardPressed]}
      onPress={() => isDone && router.push({ pathname: '/teacher-exam-overview', params: { examId: exam.id } })}
      disabled={!isDone}
    >
      ...
      <View style={styles.cardBody}>
        <Text style={styles.cardTitle} numberOfLines={1}>{exam.exam_name}</Text>
      ...
      <View style={styles.cardRight}>
        ...
        {isDone && <Ionicons name="chevron-forward" size={16} color="#CCCCCC" />}
      </View>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  countBadge: { backgroundColor: '#E1F5EE', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  ...
  cardBody: { flex: 1 },
  cardTitle: { fontSize: 14, fontWeight: '600', color: '#111111', marginBottom: 5 },
